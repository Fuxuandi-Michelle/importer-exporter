...
public class DBSurfaceOfMultiGeometry implements DBImporter {
	...
	public DBSurfaceOfMultiGeometry(Connection batchConn, Config config, DBImporterManager dbImporterManager) throws SQLException {
	...
	private void init() throws SQLException {		
		...
		.append("insert into SURFACE_OF_MULTI_GEOMETRY (ID, GMLID, ").append(gmlIdCodespace != null ? "GMLID_CODESPACE, " : "").append("PARENT_ID, ROOT_MULTI_GEOM_ID, IS_SOLID, IS_COMPOSITE, IS_TRIANGULATED, IS_XLINK, IS_REVERSE, GEOMETRY, SOLID_GEOMETRY, IMPLICIT_GEOMETRY, CITYOBJECT_ID) values ")
		...
		psNextSeqValues = batchConn.prepareStatement(dbImporterManager.getDatabaseAdapter().getSQLAdapter().getNextSequenceValuesQuery(DBSequencerEnum.SURFACE_OF_MULTI_GEOMETRY_ID_SEQ));
	...
	public long insert(AbstractGeometry surfaceGeometry, long rootMultiGeomId, long cityObjectId) throws SQLException {
		...
		insert(surfaceGeometry, surfaceGeometryId, 0, rootMultiGeomId, false, false, false, cityObjectId);
		pkManager.clear();

		return surfaceGeometryId;
	}

	public long insertImplicitGeometry(AbstractGeometry surfaceGeometry, long rootMultiGeomId) throws SQLException {
		...
		try {
			...
			dbSrid = 0;
			return insert(surfaceGeometry, rootMultiGeomId, 0);
		} finally {
			isImplicit = false;
	...
	private void insert(AbstractGeometry surfaceGeometry,
		...
		if (surfaceGeometryType == GMLClass.LINEAR_RING) {
			...
			if (origGmlId != null && !isCopy)
				dbImporterManager.putUID(origGmlId, surfaceGeometryId, rootMultiGeomId, reverse, gmlId, CityGMLClass.ABSTRACT_GML_GEOMETRY);

			psGeomElem.setLong(1, surfaceGeometryId);
			psGeomElem.setString(2, gmlId);
			psGeomElem.setLong(4, rootMultiGeomId);
		...
		// orientableSurface
			...
			if (orientableSurface.isSetBaseSurface()) {
				...
				if (surfaceProperty.isSetSurface()) {
					...
					case _TEXTURED_SURFACE:
					case ORIENTABLE_SURFACE:
						insert(abstractSurface, surfaceGeometryId, parentId, rootMultiGeomId, reverse, isXlink, isCopy, cityObjectId);
						break;
					case POLYGON:
					case COMPOSITE_SURFACE:
					case SURFACE:
						...
						if (surfacePatch.getGMLClass() == GMLClass.RECTANGLE) {
							...
							if (rectangle.isSetExterior()) {
								...
								if (exteriorLinearRing != null) {
									surfaceGeometryId = pkManager.nextId();
									insert(exteriorLinearRing, surfaceGeometryId, parentId, rootMultiGeomId, reverse, isXlink, isCopy, cityObjectId);
								}
							}
						}
		...
		// Solid
			...
			if (surfaceGeometryId == rootMultiGeomId) {
			...
			if (compositeSolid.isSetSolidMember()) {
				for (SolidProperty solidProperty : compositeSolid.getSolidMember()) {
					if (solidProperty.isSetSolid()) {
						surfaceGeometryId = pkManager.nextId();
						insert(solidProperty.getSolid(), surfaceGeometryId, parentId, rootMultiGeomId, reverse, isXlink, isCopy, cityObjectId);
					} else {
						// xlink
			...
			if (multiPolygon.isSetPolygonMember()) {
				for (PolygonProperty polygonProperty : multiPolygon.getPolygonMember()) {
					if (polygonProperty.isSetPolygon()) {
						surfaceGeometryId = pkManager.nextId();
						insert(polygonProperty.getPolygon(), surfaceGeometryId, parentId, rootMultiGeomId, reverse, isXlink, isCopy, cityObjectId);
					} else {
						// xlink
			...
			if (multiSolid.isSetSolidMembers()) {
				...
				if (solidArrayProperty.isSetSolid()) {
					for (AbstractSolid abstractSolid : solidArrayProperty.getSolid()) {
						surfaceGeometryId = pkManager.nextId();
						insert(abstractSolid, surfaceGeometryId, parentId, rootMultiGeomId, reverse, isXlink, isCopy, cityObjectId);
					}
				}
			}
		}
			...
			if (geometricComplex.isSetElement()) {
				for (GeometricPrimitiveProperty geometricPrimitiveProperty : geometricComplex.getElement()) {
					if (geometricPrimitiveProperty.isSetGeometricPrimitive())
						insert(geometricPrimitiveProperty.getGeometricPrimitive(), surfaceGeometryId, parentId, rootMultiGeomId, reverse, isXlink, isCopy, cityObjectId);
					else {
						// xlink
	...
	private void addBatch() throws SQLException {
		...
		if (++batchCounter == dbImporterManager.getDatabaseAdapter().getMaxBatchSize())
			dbImporterManager.executeBatch(DBImporterEnum.SURFACE_OF_MULTI_GEOMETRY);
	}
	...
	public DBImporterEnum getDBImporterType() {
		return DBImporterEnum.SURFACE_OF_MULTI_GEOMETRY;
	}
